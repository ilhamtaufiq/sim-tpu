...
class HomeController extends Controller
{
    ...
    public function root()
    {
        ...
        $now = Carbon::now();
        $subTahun = Registrasi::season(Carbon::now()->subYear(1))->sum(DB::raw('retribusi + ambulance'));
        $tahun = Registrasi::season($now->year)->sum(DB::raw('retribusi + ambulance'));
        $bulan = Registrasi::season($now->year)->select(
        DB::raw("DATE_FORMAT(tanggal_meninggal,'%M') as label"),
        DB::raw('sum(retribusi+ambulance) as data'), 
        )
        ->groupBy('label')
        ->pluck('data');
        $label_bulan = Registrasi::season($now->year)->select(
            DB::raw("DATE_FORMAT(tanggal_meninggal,'%M') as label"),
            DB::raw('sum(retribusi+ambulance) as data'), 
            )
            ->groupBy('label')
